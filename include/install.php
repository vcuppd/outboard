...
//
// 2012-04-25	feuerri - Replaced deprecated functions.
...
if ($submit) {   // Run this if the user pressed the Install button
  ...
  $selectDB = @mysql_select_db($db, $dbh);
  if (! $dbh || ! $selectDB) { 
    ...
    $result = @mysql_query($outboard_table,$dbh);
    if ($result) {
      echo "Done.<p>";
      echo "---------------------------------------------------<br>";
      echo "Creating <i>$logtable</i> table... ";
      $result = @mysql_query($outboard_log_table,$dbh);
      if ($result) {
        echo "Done.<p>";
        echo "---------------------------------------------------<br>";
        ...
        $result = mysql_query("INSERT into $table (userid,password,name,options) "
        		."VALUES ('$adminuser',password('$adminpass'),'$adminname','<ADMIN>')",$dbh);
        echo "Done.<p>";
